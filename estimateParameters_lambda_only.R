...
MHGibbs.lambda.only <- function(ndraws, thin.factor, init.lambda, init.beta) {
  ...
  beta <- matrix(0, nrow=ndraws, ncol=num.lags)
...
Rprof("lambdaOnly.out")
time <- system.time(draws.lambda.only <- MHGibbs.lambda.only(1000, 1, init.lambda, init.beta))
Rprof(NULL)
summaryRprof("lambdaOnly.out")
