...
cen.cont <- ifelse(colSums(gIntersects(census, houston, byid=T)) > 0, TRUE, FALSE)
...
pop <- zapsmall(FacToNum(census.layer$TotalPop[cens.index]) * census.prop, 5)

# Calculate percentage over 65
grid.areas <- grid.areas[grid.id]
grid.prop <- zapsmall(int.areas/grid.areas, 3)
pct65 <- zapsmall(FacToNum(census.layer$PCTover65[cens.index]) * grid.prop, 5)
