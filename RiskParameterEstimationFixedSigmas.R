...
MHGibbs <- function(ndraws, thin.factor, init.lstar.911, init.lstar.death, init.lstar.mu, init.beta, temp.index) {
  ...
  d.vals[1] <- 0
  temp.lvar.mu <- temp.lvar.911 <- temp.lvar.death <- 70
  lambda.var.a <- 2
  lambda.var.b <- 1
...
tm <- system.time(draws.fixed.sigma <- MHGibbs(5000, 50, init.lstar.911, init.lstar.death, init.lstar.mu, init.beta, 2))
save(draws.fixed.sigma, file="/RData/drawsFixedSigma.RData")
