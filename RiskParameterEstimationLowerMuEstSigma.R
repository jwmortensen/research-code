...
D <- function(lstar.911, lstar.death) {
  -2 * (LogLike(lstar.911, Nk.911, E.911) + 
        LogLike(lstar.death, Nk.death, E.death))
}
...
MHGibbs <- function(ndraws, thin.factor, init.lstar.911, init.lstar.death, init.lstar.mu, init.beta, temp.index) {
  ...
  for (i in 2:(ndraws*thin.factor)) {
    ...
    for (j in 1:num.blocks) {
      ...
      log.MH <- LogLike(prop.lstar.vec.911, Nk.911, E.911) - 
        LogLike(new.lstar.911, Nk.911, E.911) + 
        LogLambdaPrior(prop.lstar.vec.911, temp.lstar.mu, temp.lvar.911, lambda.inverse.matern) - 
        LogLambdaPrior(new.lstar.911, temp.lstar.mu, temp.lvar.911, lambda.inverse.matern)
    ...
    for (j in 1:num.blocks) {
      ...
      log.MH <- LogLike(prop.lstar.vec.death, Nk.death, E.death) - 
        LogLike(new.lstar.death, Nk.death, E.death) +
        LogLambdaPrior(prop.lstar.vec.death, temp.lstar.mu, temp.lvar.death, lambda.inverse.matern) - 
        LogLambdaPrior(new.lstar.death, temp.lstar.mu, temp.lvar.death, lambda.inverse.matern)
    ...
    for (j in 1:num.blocks) {
      ...
      log.MH <- LogLambdaPrior(temp.lstar.911, prop.lstar.vec.mu, temp.lvar.911, lambda.inverse.matern) - 
        LogLambdaPrior(temp.lstar.911, new.lstar.mu, temp.lvar.911, lambda.inverse.matern) +
        LogLambdaPrior(temp.lstar.death, prop.lstar.vec.mu, temp.lvar.death, lambda.inverse.matern) -
        LogLambdaPrior(temp.lstar.death, new.lstar.mu, temp.lvar.death, lambda.inverse.matern) + 
    ...
    if (i %% thin.factor == 0) {
      ...
      beta[n, ] <- temp.beta
      d.vals[n] <- D(temp.lstar.911, temp.lstar.death)
    }
...
tm <- system.time(draws.lower.mu <- MHGibbs(1000, 1, init.lstar.911, init.lstar.death, init.lstar.mu, init.beta, 2))
...
# }
# 
# lstar.mu <- apply(draws$lstar.mu[-c(1:6000),], 2, mean)
# lambda.911 <- calcLambda(draws$lstar.911[-c(1:6000), ], E.911, draws$lstar.mu[-c(1:6000), ])
# lambda.death <- calcLambda(draws$lstar.death[-c(1:6000), ], E.death, draws$lstar.mu[-c(1:6000), ])
# lstar.911 <- apply(draws$lstar.911[-c(1:6000), ], 2, mean)
# lstar.death <- apply(draws$lstar.death[-c(1:6000), ], 2, mean)
